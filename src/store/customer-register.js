...
const useCustomerRegisterStore = defineStore({
    ...
    actions: {
        ...
        async createUser() {
            ...
            const app = useAppStore()
            this.loading = true
            const result = await API.createCustomer(this.form)
            if (!result.success) {
                this.loading = false
...
export function useCustomerRegister() {
    const customerRegister = useCustomerRegisterStore()

    const { busy, loading } = storeToRefs(customerRegister)
    const { getFormData, createUser, resetForm } = customerRegister
...
export function useCustomerRegisterForm() {
    const customerRegister = useCustomerRegisterStore()

    const { form, error, formData } = storeToRefs(customerRegister)
    const { handleInput } = customerRegister
